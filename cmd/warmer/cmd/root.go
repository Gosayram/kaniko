...
// Package cmd provides the command-line interface for the kaniko cache warmer.
package cmd
...
// RootCmd is the main command for the kaniko cache warmer.
var RootCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, _ []string) {
		if _, err := os.Stat(opts.CacheDir); os.IsNotExist(err) {
			...
			err = os.MkdirAll(opts.CacheDir, cacheDirPerm)
			if err != nil {
...
func addKanikoOptionsFlags() {
	...
	RootCmd.PersistentFlags().VarP(&opts.InsecureRegistries, "insecure-registry", "",
		"Insecure registry using plain HTTP to pull. Set it repeatedly for multiple registries.")
	RootCmd.PersistentFlags().VarP(&opts.SkipTLSVerifyRegistries, "skip-tls-verify-registry", "",
		"Insecure registry ignoring TLS verify to pull. Set it repeatedly for multiple registries.")
