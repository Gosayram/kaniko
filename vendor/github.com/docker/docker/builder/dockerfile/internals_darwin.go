...
// parseChownFlag parses the chown flag for macOS.
// On macOS, we use a simplified approach since we don't have access
// to container filesystem passwd/group files like on Linux.
func parseChownFlag(ctx context.Context, builder *Builder, state *dispatchState, chown, ctrRootPath string, identityMapping user.IdentityMapping) (identity, error) {
	...
	// On macOS, we can't easily access container filesystem files,
	// so we use a simplified approach with numeric IDs
...
func parseUserID(userStr string) (int, error) {
	...
	// For macOS, we support common user names with fallback to numeric parsing
...
func parseGroupID(groupStr string) (int, error) {
	...
	// For macOS, we support common group names with fallback to numeric parsing
